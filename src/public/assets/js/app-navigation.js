...
(function () {
    ...
    function searchByPrefix() {
        console.log(
            "searchByPrefix called, searchPrefix=",
            state.searchPrefix,
            "pathPrefix=",
            state.pathPrefix,
        );
    ...
    async function refresh() {
        ...
        try {
            ...
            const items = (data.items || []).map((it) => {
                if (it.type === "prefix") {
                    ...
                    return {
                        type: "prefix",
                        name:
                            it.name || relPrefix.split("/").slice(-2)[0] + "/",
                        prefix: relPrefix,
                        size: 0,
                        dateModified: null,
                    };
            ...
            const filtered = items.filter((row) => {
                ...
                return !config.keyExcludePatterns.find((rx) =>
                    rx.test(String(keyLike).replace(/^\//, "")),
                );
            });
            ...
            for (const it of filtered) {
                const id =
                    it.type === "prefix" ? "P:" + it.prefix : "F:" + it.key;
