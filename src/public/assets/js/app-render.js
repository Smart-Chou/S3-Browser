...
(function () {
  ...
  function renderListView() {
    ...
    state.pathContentTableData.forEach(row => {
      ...
      if (row.type === 'content') {
        nameSpan.addEventListener('click', () => BB.events.openPreview(row));
      } else {
        nameSpan.addEventListener('click', () => BB.nav.goToPrefix(row.prefix));
      }
      ...
      if (row.dateModified) {
        ...
        span.textContent = utils.formatDateTime_relative(row.dateModified);
      ...
      menuDiv.setAttribute(row.type === 'content' ? 'data-key' : 'data-prefix',
      ...
      if (row.type === 'content') {
        ...
        downloadItem.addEventListener('click', () => BB.events.onRowDownload(row));
    ...
    state.pathContentTableData.forEach(row => {
      ...
      if (row.type === 'content') {
        ...
        if (row.dateModified) {
          ...
          timeSpan.textContent = utils.formatDateTime_relative(row.dateModified);
