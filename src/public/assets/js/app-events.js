...
(function () {
    ...
    function onPrefixDetails(row) {
        const prefixAbs = ((config.rootPrefix || "") + row.prefix).replace(
            /\/{2,}/g,
            "/",
        );
    ...
    async function downloadAllFiles() {
        ...
        const archiveName = (state.pathPrefix || "")
            .split("/")
            .filter((p) => p.trim())
            .pop();
        const archiveData = [];
        const archive = new Zip((err, data) => {
            if (err) throw err;
            archiveData.push(data);
        });

        await Promise.all(
            archiveFiles.map(async (url) => {
                const fileName = url
                    .split("/")
                    .filter((p) => p.trim())
                    .pop();
                ...
                while (true) {
                    ...
                    const p1 = totalContentLength
                        ? totalReceivedLength / totalContentLength
                        : 0;
                    const p2 = state.downloadAllFilesCount
                        ? state.downloadAllFilesReceivedCount /
                          state.downloadAllFilesCount
                        : 0;
        ...
        state.downloadAllFilesCount =
            state.downloadAllFilesReceivedCount =
            state.downloadAllFilesProgress =
                null;
    }
    ...
    function toggleDropdown(name) {
        const actionDropdown =
            elements.actionsDropdownButton.closest(".toolbar-dropdown");
        const newDropdown =
            elements.newDropdownButton.closest(".toolbar-dropdown");
    ...
    function setupEventListeners() {
        ...
        // 下拉菜单
        elements.actionsDropdownButton.addEventListener("click", () =>
            toggleDropdown("actions"),
        );
        elements.newDropdownButton.addEventListener("click", () =>
            toggleDropdown("new"),
        );

        // 操作按钮
        elements.downloadAllButton.addEventListener("click", downloadAllFiles);
        elements.folderDetailsButton.addEventListener(
            "click",
        ...
        elements.listViewButton.addEventListener("click", () =>
            render.setViewMode("list"),
        );
        elements.gridViewButton.addEventListener("click", () =>
            render.setViewMode("grid"),
        );
        ...
        const dropdownMenus = [
            elements.actionsDropdownMenu,
            elements.newDropdownMenu,
        ];
