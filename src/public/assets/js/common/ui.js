...
(function () {
    ...
    function buildModal({
        ...
        const btnClose = el(
            "button",
            "bb-btn bb-btn-ghost bb-modal-x",
        ...
        const btnOk = el(
            ...
            kind === "confirm" ? "OK" : kind === "prompt" ? "OK" : "OK",
        );
        if (kind != "alert") {
    ...
    async function alert({ title = "", message = "", html = "" }) {
        return new Promise((resolve) => {
            const { overlay, btnOk, btnClose } = buildModal({
                title,
                message,
                html,
                kind: "alert",
    ...
    async function prompt({ title = "", message = "", defaultValue = "" }) {
        return new Promise((resolve) => {
            const { overlay, body, btnOk, btnCancel, btnClose } = buildModal({
                title,
                message,
                kind: "prompt",
                defaultValue,
