...
(function () {
    ...
    const elements = {
        ...
        actionsDropdownButton: document.getElementById(
    ...
    function isImageExt(e) {
        return BB.detect.isImageExt(e);
    }
    function isArchiveExt(e) {
        return BB.detect.isArchiveExt(e);
    }
    function isVideoExt(e) {
        return BB.detect.isVideoExt(e);
    }
    function isAudioExt(e) {
        return BB.detect.isAudioExt(e);
    }
    function isSpreadsheetExt(e) {
        return BB.detect.isSpreadsheetExt(e);
    }
    function isPresentationExt(e) {
        return BB.detect.isPresentationExt(e);
    }
    function isPdfExt(e) {
        return BB.detect.isPdfExt(e);
    }
    function isCodeExt(e) {
        return BB.detect.isCodeExt(e);
    }
    ...
    function validBucketPrefix(prefix) {
        ...
        if (prefix.startsWith("/") && bucketPrefix().includes("/"))
            return false;
        return true;
    }
    ...
    function canDownloadAll() {
        const filesCount = state.pathContentTableData.filter(
            (i) => i.type === "content",
        ).length;
    ...
    function breadcrumbs() {
        ...
        if (root && p.startsWith(root))
            p = p.slice(root.length).replace(/^\/+/g, "");
    ...
    (function setup() {
        ...
        if (config.title)
            config.titleHTML = config.title.startsWith(htmlPrefix)
                ? config.title.substring(htmlPrefix.length)
                : config.title.escapeHTML();
        if (config.subtitle)
            config.subtitleHTML = config.subtitle.startsWith(htmlPrefix)
                ? config.subtitle.substring(htmlPrefix.length)
                : config.subtitle.escapeHTML();
        ...
        if (config.rootPrefix)
            config.rootPrefix = config.rootPrefix.replace(/\/?$/, "/");
        ...
        const absTrash =
            (config.rootPrefix || "") + (config.trashPrefix || "_trash/");
        const rx = new RegExp(
            "^" + absTrash.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),
        );
        if (
            !config.keyExcludePatterns.some(
                (r) => r.toString() === rx.toString(),
            )
        ) {
